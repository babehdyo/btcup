...
class Donchian(IndicatorUtils, HeikinAshi):
    ...
    def analyze(self, historical_data, signal=['LONG','END LONG'], hot_thresh=None, cold_thresh=None, period_count=30):
...
#  set current HH to previous HH value +1. Otherwise 0
        ...
        data = [{'LONG': False, 'END LONG':False, 'is_hot': False, 'is_cold': False}]
        sign = pandas.DataFrame(data)

        if olong[0] == 1:
            sign['LONG'] = True
            sign['is_hot'] = True
        if olong[1] > 0 and olong[0] == 0:
            sign['END LONG'] = True
            sign['is_cold'] = True
